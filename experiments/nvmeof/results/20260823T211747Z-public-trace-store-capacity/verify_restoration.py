...
service_before_path = next(inventory.glob("*.target-service.before.txt"))
prefix = service_before_path.name.removesuffix(".target-service.before.txt")
...
service_before = service_before_path.read_text(encoding="utf-8")
service_after = text(f"{prefix}.target-service.after.txt")
subsystems_before = value(f"{prefix}.target-subsystems.before.json")
subsystems_after = value(f"{prefix}.target-subsystems.after.json")
bdevs_before = value(f"{prefix}.target-bdevs.before.json")
bdevs_after = value(f"{prefix}.target-bdevs.after.json")

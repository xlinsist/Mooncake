...
run_root="$base/$RUN_ID-public-trace-store"
...
convert_cell() {
  ...
  python3 "$scripts/public_trace_workload.py" "$source" "$cell" \
    ...
    --run-id "$RUN_ID-$scenario-trial$trial" \
...
run_case() {
  ...
  local command=(python3 "$scripts/kv_workload.py" replay \
    ...
    --run-id "$RUN_ID-$scenario-trial$trial" --recompute-us 1000)
...
recovery_smoke() {
  ...
  python3 "$scripts/kv_workload.py" generate "$dir" \
    --requests 1 --blocks-per-request 1 --block-size 131072 \

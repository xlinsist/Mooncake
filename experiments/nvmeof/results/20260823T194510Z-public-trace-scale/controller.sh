...
target_image=/sharenvme/userhome/zhouxl/mooncake-public-trace-${RUN_ID}.img
target_bdev=TraceScale${RUN_ID//[^[:alnum:]]/}
target_nqn=nqn.2026-08.local.mooncake:trace-scale-${RUN_ID}
target_serial=TS${RUN_ID//[^[:digit:]]/}
target_serial=${target_serial:0:20}
client_mount=/mnt/mooncake-trace-${RUN_ID}
client_remote_storage=${client_mount}/bench
client_local_storage=/mnt/datassd/zhouxulin-minipool/transparent-phase4/public-trace-scale-${RUN_ID}
client_result=/sharenvme/userhome/zhouxulin/mooncake-public-trace-results/${RUN_ID}-public-trace-scale
benchmark_root=/sharenvme/userhome/zhouxulin/mooncake-kv-e82f0bb7/benchmarks/storage_benchmark_v1
trace_root=/sharenvme/userhome/zhouxulin/mooncake-kv-e82f0bb7/FAST25-release/traces
controller_log=/tmp/mooncake-public-trace-scale-${RUN_ID}.log
service_pid=
created_bdev=0
created_subsystem=0
connected_client=0
...
client_cmd bash -s -- "$client_result" "$benchmark_root" "$trace_root" "$RUN_ID" "$target_nqn" <<'EOF'
set -Eeuo pipefail
result=$1
benchmark_root=$2
trace_root=$3
...
target_nqn=$5
{
  ...
  id
  uname -a
  git -C "$(dirname "$(dirname "$benchmark_root")")" rev-parse HEAD
...
cat >"/tmp/${RUN_ID}.design.json" <<EOF
{
  ...
  "threads": 1,
  "replay_scale": 0,
  "claim_boundary": "GPU-free durable storage-path scale evidence; different devices/filesystems, sequential single-thread replay, modulo page mapping, no transport-only, serving, true-concurrency, or system-superiority claim."
}
EOF
...
created_bdev=1
target_rpc_cmd nvmf_create_subsystem "$target_nqn" -a -s "$target_serial" -d MooncakeTraceScale
created_subsystem=1
...
echo "run 36 durable cases"
for trial in 1 2 3; do
  case $trial in
    1) scenarios=(conversation toolagent); scales=(20 50 100) ;;
    2) scenarios=(toolagent conversation); scales=(50 100 20) ;;
    3) scenarios=(conversation toolagent); scales=(100 20 50) ;;
  esac
  scenario_index=0
  for scenario in "${scenarios[@]}"; do
    scale_index=0
    for requests in "${scales[@]}"; do
      if (( (trial + scenario_index + scale_index) % 2 )); then paths=(local remote); else paths=(remote local); fi
      for path in "${paths[@]}"; do
        case_id=${scenario}-r${requests}-t${trial}-${path}
        echo "case $case_id"
        run_case "$scenario" "$requests" "$trial" "$path" "$case_id"
      done
...
client_cmd test "$(client_cmd find "$client_result/raw" -name '*.rc' -type f | wc -l)" -eq 36

...
[[ $REPLAY_SCALE =~ ^[0-9]+([.][0-9]+)?$ ]]
scale_tag=${REPLAY_SCALE//./p}
run_root="$base/$RUN_ID-public-trace-store-r${REQUESTS}-c${CAPACITY_PAGES}-s${scale_tag}"
...
pacing_enabled=$(python3 - "$REPLAY_SCALE" <<'PY'
import math, sys
scale = float(sys.argv[1])
if not math.isfinite(scale) or scale < 0:
    raise SystemExit("REPLAY_SCALE must be finite and non-negative")
print(int(scale > 0))
PY
)
[[ -d $scripts ]]
...
convert_cell() {
  ...
  mkdir -p "$cell"
  local command=(python3 "$scripts/public_trace_workload.py" "$source" "$cell" \
    --requests "$REQUESTS" --capacity-pages "$CAPACITY_PAGES" \
    --block-size "$BLOCK_SIZE" \
    --run-id "$RUN_ID-r$REQUESTS-c$CAPACITY_PAGES-s$scale_tag-$scenario-trial$trial")
  (( pacing_enabled )) && command+=(--preserve-arrivals)
  "${command[@]}" \
...
run_case() {
  ...
  local command=(python3 "$scripts/kv_workload.py" replay \
    ...
    --run-id "$RUN_ID-r$REQUESTS-c$CAPACITY_PAGES-s$scale_tag-$scenario-trial$trial" \
    --recompute-us 1000 --replay-scale "$REPLAY_SCALE")
...
cat >"$out/design.json" <<EOF
{
  ...
  "preserve_arrivals": $( (( pacing_enabled )) && printf true || printf false ),
  "trials": 3,
  ...
  "claim_boundary": "Sequential FAST'25 hash-id replay with compressed source arrival pacing through Mooncake Store; no true concurrency, model serving, transport-only, saturation, or system-superiority claim."
}
EOF

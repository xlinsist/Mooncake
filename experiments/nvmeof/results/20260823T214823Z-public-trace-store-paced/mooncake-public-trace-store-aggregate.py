...
pacing_errors = []
for row in rows:
    if float(row["replay_scale"]) != float(design["replay_scale"]):
        pacing_errors.append(f"{row['scenario']}-trial{row['trial']}-{row['case_id']}: replay scale mismatch")
    for field in ("scheduled_span_us", "processing_wall_us", "completion_lag_us"):
        if row[field] == "":
            pacing_errors.append(f"{row['scenario']}-trial{row['trial']}-{row['case_id']}: missing {field}")
matrix = {
    "status": "pass",
    "planned_cases": 30,
    ...
    "observed_replay_scales": observed_replay_scales,
...
if (
    ...
    or observed_replay_scales != [float(design["replay_scale"])]
    or pacing_errors

...
def test_add_spdk_telemetry(tmp_path: Path):
    ...
    before.write_text(json.dumps({
        "tick_rate": 1000,
        "bdevs": [{"name": "disk", "bytes_read": 0, "bytes_written": 0,
                   "read_latency_ticks": 0, "write_latency_ticks": 0}],
    }))
    after.write_text(json.dumps({
        "tick_rate": 1000,
        "bdevs": [{"name": "disk", "bytes_read": 2000, "bytes_written": 0,
                   "read_latency_ticks": 500, "write_latency_ticks": 0}],
    }))
